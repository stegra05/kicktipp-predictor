...
class PerformanceTracker:
    ...
    def _update_performance_stats(self):
        ...
        performance_summary = {
            ...
            "exact_score_rate": points_distribution[4] / total_matches
            if total_matches > 0
            else 0,
    ...
    def get_current_stats(self) -> dict:
        ...
        return {
            ...
            "exact_score_percentage": (points_dist[4] / total_matches * 100)
            if total_matches > 0
            else 0,
            "difference_percentage": (points_dist[3] / total_matches * 100)
            if total_matches > 0
            else 0,
            "result_percentage": (points_dist[2] / total_matches * 100)
            if total_matches > 0
            else 0,
            "incorrect_percentage": (points_dist[0] / total_matches * 100)
            if total_matches > 0
            else 0,
        }
    ...
    def print_summary(self):
        ...
        print(
            f"  Exact Scores (4pts):      {stats['exact_scores']:3d} ({stats['exact_score_percentage']:5.1f}%)"
        )
        print(
            f"  Correct Differences (3pts): {stats['correct_differences']:3d} ({stats['difference_percentage']:5.1f}%)"
        )
        print(
            f"  Correct Results (2pts):   {stats['correct_results']:3d} ({stats['result_percentage']:5.1f}%)"
        )
        print(
            f"  Incorrect (0pts):         {stats['incorrect']:3d} ({stats['incorrect_percentage']:5.1f}%)"
        )
        print()
        ...
        if stats["total_predictions"] > 0:
            ...
            print(
                f"Projected Season Total (38 matchdays): {projected_season:.1f} points"
            )

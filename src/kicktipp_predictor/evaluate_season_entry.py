...
def run(dynamic: bool = False, retrain_every: int = 1) -> None:
    ...
    if len(historical_finished) < 50:
        ...
        historical_finished.extend(
            [m for m in prev_season_matches if m.get("is_finished")]
        )
    ...
    if not dynamic:
        ...
        for matchday in range(first_matchday, last_matchday + 1):
            ...
            matchday_matches = [
                m for m in finished_matches if m["matchday"] == matchday
            ]
            ...
            print(
                f"Generated and evaluated {len(predictions)} predictions for matchday {matchday}"
            )
    else:
        ...
        print(
            f"Initialized training set with {len(cumulative_training_matches)} matches from previous seasons."
        )
        ...
        for matchday in range(first_matchday, last_matchday + 1):
            ...
            if (matchday - first_matchday) % max(1, int(retrain_every)) == 0:
                print(
                    f"Retraining model with {len(cumulative_training_matches)} total matches..."
                )
            ...
            matchday_matches_to_predict = [
                m for m in evaluation_season_matches if m["matchday"] == matchday
            ]
            ...
            features_df = data_loader.create_prediction_features(
                matchday_matches_to_predict, cumulative_training_matches
            )
    ...
    if use_dynamic_preds:
        ...
        ah = np.asarray(
            [int(p.get("actual_home_score", 0)) for p in preds_all], dtype=int
        )
        aa = np.asarray(
            [int(p.get("actual_away_score", 0)) for p in preds_all], dtype=int
        )
        y_true = []
        ...
        id_to_actual = {
            m["match_id"]: (m["home_score"], m["away_score"])
    ...
    if use_dynamic_preds:
        ...
        features_like = (
            features_all.copy() if "features_all" in locals() else pd.DataFrame()
        )
    ...
    print(
        ...
        + "  ".join(
            [
                f"{lab}={lab_counts.get(lab,0)} ({lab_counts.get(lab,0)/total_labels:.2%})"
    ...
    print(
        ...
        + "  ".join(
            [
                f"{lab}={pred_counts.get(lab,0)} ({pred_counts.get(lab,0)/max(1,len(pred_labels)):.2%})"
                for lab in LABELS_ORDER
            ]
        )
    )
    ...
    print("-" * 80)
    print(
        f"avg_pts={metrics['avg_points']:.3f}  acc={metrics['accuracy']:.3f}  "
    ...
    print(
        "Counts by points    : "
        + ", ".join([f"{p}p={pt_counts[p]}" for p in unique_pts])
    )
    print(
        f"Avg points          : {np.mean(pts) if len(pts) else 0.0:.3f}  Total: {int(np.sum(pts))}"
    )
    ...
    print(
        f"Correct differences : {correct_diffs:3d} ({correct_diffs/len(pts)*100:.1f}%)"
    )
    print(
        f"Correct results     : {correct_results:3d} ({correct_results/len(pts)*100:.1f}%)"
    )
    ...
    try:
        plot_confidence_buckets(
            conf_df, os.path.join(out_dir, "confidence_buckets_season.png")
        )
    except Exception:
        pass
    if len(conf_df) > 0:
        ...
        for _, r in conf_df.iterrows():
            print(
                f"bin={r['bin']:<14} count={int(r['count']):4d}  avg_pts={float(r['avg_points']):.3f}  "
    ...
    try:
        ...
        plot_reliability_curve(
            curve_H, "H", os.path.join(out_dir, "calibration_home_season.png")
        )
        plot_reliability_curve(
            curve_D, "D", os.path.join(out_dir, "calibration_draw_season.png")
        )
        plot_reliability_curve(
            curve_A, "A", os.path.join(out_dir, "calibration_away_season.png")
        )
    except Exception:
        pass
    ...
    with open(
        os.path.join(out_dir, "metrics_table_season.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(
            f"main      avg_pts={metrics['avg_points']:.3f}  acc={metrics['accuracy']:.3f}  "
            f"brier={metrics['brier']:.4f}  logloss={metrics['log_loss']:.4f}  rps={metrics['rps']:.4f}\n"
        )
    ...
    for i in range(len(preds_all)):
        ...
        mid_val = (
            features_like.iloc[i]["match_id"]
            if "match_id" in features_like.columns
            else None
        )
        md_val = (
            features_like.iloc[i]["matchday"]
            if "matchday" in features_like.columns
            else None
        )
        ht_val = (
            features_like.iloc[i]["home_team"]
            if "home_team" in features_like.columns
            else None
        )
        at_val = (
            features_like.iloc[i]["away_team"]
            if "away_team" in features_like.columns
            else None
        )
        debug_rows.append(
            {
                "match_id": int(mid_val)
                if mid_val is not None and not pd.isna(mid_val)
                else None,
                "matchday": int(md_val)
                if md_val is not None and not pd.isna(md_val)
                else None,
                "home_team": ht_val,
                "away_team": at_val,
                "actual": actual_lab,
    ...
    pd.DataFrame(debug_rows).to_csv(
        os.path.join(out_dir, "debug_season.csv"), index=False
    )
    ...
    for md in matchdays:
        ...
        acc = (
            float(np.mean(np.argmax(P_md, axis=1) == y_idx_md))
            if n_md
            else float("nan")
        )
        ...
        mae_gd = (
            float(np.mean(np.abs((ph_md - pa_md) - (ah_md - aa_md))))
            if n_md
            else float("nan")
        )
    ...
    if len(rows) > 0:
        ...
        print("-" * 80)
        print(
            f"{'MD':>3}  {'n':>2}  {'avg':>5}  {'tot':>4}  {'base':>5}  {'Δavg':>5}  {'acc':>5}  {'ex/diff/res':>10}"
        )
        for r in sorted(rows, key=lambda x: x["matchday"]):
            ex_dr = f"{r['exact_count']}/{r['diff_count']}/{r['result_count']}"
            print(
                f"{int(r['matchday']):3d}  {int(r['n']):2d}  {r['avg_points']:.2f}  {int(r['total_points']):4d}  "
    ...
    if plt is not None and len(matchdays) == len(avg_pts_series):
        try:
            ...
            ax.plot(
                matchdays, avg_pts_series_baseline, label="Baseline (2-1 H)", marker="o"
            )
            ...
            ax2.plot(
                matchdays, cum_pts_series_baseline, label="Baseline (2-1 H)", marker="o"
            )

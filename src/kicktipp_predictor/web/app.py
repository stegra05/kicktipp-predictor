    ...
    try:
        ...
        if not upcoming_matches:
            return jsonify(
                {
                    "success": True,
                    "message": f"No upcoming matches in the next {days} days",
                    "predictions": [],
                }
            )
            ...
            formatted_predictions.append(
                {
                    ...
                    if pred.get("match_id") is not None
                    else None,
                    ...
                    "home_win_probability": float(
                        round(float(pred["home_win_probability"]) * 100, 1)
                    ),
                    "draw_probability": float(
                        round(float(pred["draw_probability"]) * 100, 1)
                    ),
                    "away_win_probability": float(
                        round(float(pred["away_win_probability"]) * 100, 1)
                    ),
                    "confidence": float(round(float(pred["confidence"]) * 100, 1)),
                    "home_expected_goals": float(
                        round(float(pred["home_expected_goals"]), 2)
                    ),
                    "away_expected_goals": float(
                        round(float(pred["away_expected_goals"]), 2)
                    ),
                }
            )
...
def get_table():
    ...
    try:
        ...
        for team, data in table.items():
            # Get Form
            team_history = [
                m for m in finished_matches if team in (m["home_team"], m["away_team"])
            ]
            form_features = data_loader._get_form_features(
                team, team_history, "team", last_n=5
            )

            form_string = ""
            recent_matches = sorted(
                team_history, key=lambda x: x["date"], reverse=True
            )[:5]
            ...
            # Get EWMA points
            ewma_points = (
                latest_ewma.loc[team]["points_ewm5"]
                if team in latest_ewma.index
                else 0.0
            )
        ...
        return jsonify({"success": True, "table": sorted_table})
    ...
    try:
        ...
        if not os.path.exists(metrics_path):
            return jsonify(
                {
                    "success": False,
                    "error": "Metrics file not found. Run evaluation first.",
                }
            )
...
def get_match_detail(match_id):
    ...
    try:
        ...
        features_df = data_loader.create_prediction_features([match_data], all_matches)
        ...
        formatted_prediction = {
            ...
            "home_win_probability": float(
                round(float(prediction["home_win_probability"]) * 100, 1)
            ),
            "draw_probability": float(
                round(float(prediction["draw_probability"]) * 100, 1)
            ),
            "away_win_probability": float(
                round(float(prediction["away_win_probability"]) * 100, 1)
            ),
            "confidence": float(round(float(prediction["confidence"]) * 100, 1)),
            "home_expected_goals": float(
                round(float(prediction["home_expected_goals"]), 2)
            ),
            "away_expected_goals": float(
                round(float(prediction["away_expected_goals"]), 2)
            ),

...
class MatchPredictor:
    ...
    def train(self, matches_df: pd.DataFrame):
        ...
        if len(training_data) < self.config.model.min_training_matches:
            self._log(
                f"Not enough training data. Need at least {self.config.model.min_training_matches} matches."
            )
            return
        ...
        self.feature_columns = potential_features.select_dtypes(
        ...
        if use_time_decay and "date" in training_data.columns:
            ...
            half_life = float(
                getattr(self.config.model, "time_decay_half_life_days", 90.0)
            )
        ...
        self._log(
            ...
            {k: f"{int(v)} ({v / total:.1%})" for k, v in counts.items()},
        )
        ...
        self._train_goal_models(
            X, y_home, y_away, sample_weights=time_weights_all, dates=dates_series
        )
    ...
    def _train_goal_models(
        ...
        if dates is not None and len(dates) == len(X):
            try:
                ...
                tr_idx, val_idx = train_test_split(
                    idx_all, test_size=0.1, random_state=self.config.model.random_state
                )
        ...
        self.home_goals_model.set_params(
            eval_metric="poisson-nloglik", early_stopping_rounds=esr
        )
        self.away_goals_model.set_params(
            eval_metric="poisson-nloglik", early_stopping_rounds=esr
        )

        if sw_tr is not None:
            self.home_goals_model.fit(
                X_tr,
                yh_tr,
                sample_weight=sw_tr,
            ...
            self.home_goals_model.fit(
                X_tr, yh_tr, eval_set=[(X_val, yh_val)], verbose=False
            )
            self.away_goals_model.fit(
                X_tr, ya_tr, eval_set=[(X_val, ya_val)], verbose=False
            )
    ...
    def _train_outcome_model(
        self,
        X: pd.DataFrame,
        y_result_encoded: np.ndarray,
        time_weights_all: np.ndarray,
    ):
    ...
    def predict(
        ...
        missing_columns = [
            c for c in self.feature_columns if c not in available_columns
        ]
        ...
        if temp != 1.0:
            with np.errstate(over="ignore"):
                ...
                classifier_probs = classifier_probs / classifier_probs.sum(
                    axis=1, keepdims=True
                )

        # Prior blending only when using classifier source (avoid double blending)
        prob_source = str(
            getattr(self.config.model, "prob_source", "classifier")
        ).lower()
        alpha = (
            float(getattr(self.config.model, "prior_blend_alpha", 0.0))
        ...
        if alpha > 0.0:
            try:
                ...
                classifier_probs = (1.0 - alpha) * classifier_probs + alpha * prior[
                    None, :
                ]
                classifier_probs = classifier_probs / classifier_probs.sum(
                    axis=1, keepdims=True
        ...
        if workers is not None and workers > 1 and n > 0:
            ...
            scorelines = [
                compute_scoreline_for_outcome(
                    outcomes[i],
                    float(home_lambdas[i]),
                    float(away_lambdas[i]),
                    max_goals,
                )
                for i in range(n)
            ]
        ...
        proba_grid_G = int(
            getattr(self.config.model, "proba_grid_max_goals", max_goals)
        )
        ...
        if prob_source == "classifier":
            ...
            final_probs = (
                poisson_probs if poisson_probs is not None else classifier_probs
            )
        else:  # 'hybrid'
        ...
        for idx in range(n):
            ...
            probs_sorted = sorted(
                [home_win_prob, draw_prob, away_win_prob], reverse=True
            )
